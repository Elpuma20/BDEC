...
async function reseed() {
    try {
        ...
        console.log('Cleaning jobs and applications...');
        await db.run('DELETE FROM applications');
        await db.run('DELETE FROM jobs');
        await db.run('DELETE FROM sqlite_sequence WHERE name="jobs" OR name="applications"');

        console.log('Running seed...');
        // We'll import the seed logic or just re-run it since it's already updated in seed.ts
        // But for a standalone reseed, it's safer to have the logic here or call seed() if exported
    } catch (error) {
        console.error('Error during reseed:', error);
    }
}

// Since seed isn't exported in seed.ts, I'll just put the logic here for the reseed script
// to ensure it works correctly as a standalone tool.
